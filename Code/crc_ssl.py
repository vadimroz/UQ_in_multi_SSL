...
from Code.utilities import create_save_directory, normalize, calc_angular_error, local_sharpness, calc_azi_ele_err_degrees, \
    get_neighborhood
...
class CoverageSet:
    ...
    def test(self, test_sets:int, estimated_positions, true_positions, likelihood_maps, significance_level: np.array,
             test_plot: bool = False, only_peak=False, collect_angular_err=False, collect_peaks=False,
             collect_entropy=False, collect_MAE_per_axis=False):
        ...
        if collect_angular_err:
            self._angular_errors = [[[] for _ in range(n)] for _ in range(self._speakers)]
            metrics['angular_error'] = self._angular_errors
            metrics['area'] = self._test_area

        if collect_peaks:
            self._peaks = [[[] for _ in range(n)] for _ in range(self._speakers)]
            metrics['peaks'] = self._peaks

        if collect_entropy:
            self._entropy = [[[] for _ in range(n)] for _ in range(self._speakers)]
            metrics['entropy'] = self._entropy

        if collect_MAE_per_axis:
            self._mae_azimuth = [[[] for _ in range(n)] for _ in range(self._speakers)]
            self._mae_elevation = [[[] for _ in range(n)] for _ in range(self._speakers)]
            metrics['mae_azimuth'] = self._mae_azimuth
            metrics['mae_elevation'] = self._mae_elevation

        for level_ind, level in enumerate(significance_level):

            lambda_ = self._calc_conformal_risk_control(level)
            if lambda_.shape[0] == 1:
                lambda_ = np.repeat(lambda_, self._speakers)

            for lmbd_indx, lmbd in enumerate(lambda_):
                print(f'Conformal Risk Control: (error rate:{level}, Speaker: {lmbd_indx}: lambda:{lmbd:.3f})')
            ...
            for set_index in tqdm(range(test_sets), desc='Testing coverage set'):
                ...
                for estimated_speaker, true_speaker in zip(estimated_speaker_order, true_speaker_order):
                    ...
                    orig_map = likelihood_maps[set_index, 0, ...]
                    likelihood_map = normalize(likelihood_map)
                    ...
                    self._test_coverage[estimated_speaker][level_ind].append(coverage_set[true_source_grid[0], true_source_grid[1]])
                    self._test_area[estimated_speaker][level_ind].append(np.sum(coverage_set) * self.room.resolution)

                    if collect_angular_err:
                        ang_err = calc_angular_error(estimated_positions[set_index, estimated_speaker], true_positions[set_index, true_speaker])
                        self._angular_errors[estimated_speaker][level_ind].append(ang_err)

                    if collect_peaks:
                        peak_sharpness, _ = local_sharpness(orig_map, estimated_source_grid[0], estimated_source_grid[1])
                        self._peaks[estimated_speaker][level_ind].append(peak_sharpness)

                    if collect_entropy:
                        quant = np.quantile(orig_map, 0.96)
                        lk_map = np.where(likelihood_map <= quant, 1e-12, likelihood_map)
                        prob_map = lk_map / np.sum(lk_map)
                        entropy = -np.sum(prob_map * np.log(prob_map + 1e-12))
                        self._entropy[estimated_speaker][level_ind].append(entropy)

                    if collect_MAE_per_axis:
                        azi_err, elev_err = calc_azi_ele_err_degrees(estimated_positions[set_index, estimated_speaker], true_positions[set_index, true_speaker])
                        self._mae_azimuth[estimated_speaker][level_ind].append(azi_err)
                        self._mae_elevation[estimated_speaker][level_ind].append(elev_err)
                ...
                if test_plot:
                    figure_path = os.path.join(current_path, f'coverage_{int((1-level)*100)}.png')
        ...
        if metrics:
            if collect_MAE_per_axis:
                metrics['mae_azimuth'] = np.mean(self._mae_azimuth, axis=2)
                metrics['mae_elevation'] = np.mean(self._mae_elevation, axis=2)
            return np.mean(np.array(self._test_coverage), axis=2), np.mean(np.array(self._test_area), axis=2), metrics
        else:
            return np.mean(np.array(self._test_coverage), axis=2), np.mean(np.array(self._test_area), axis=2)#, coverage_set_maps
