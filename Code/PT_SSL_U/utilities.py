...
def compute_p_values_wsr(losses, combinations, alphas, delta, Kmax, scale=1.0):
    p_values = []
    ...
    for combi_index in combinations:
        ...
        pval_md = wsr_p_value(batch_MD_per, alphas[0], delta, A=0, B=Kmax, scale=scale)
        pval_mc = wsr_p_value(batch_MC_per, alphas[1], delta, A=0, B=Kmax, scale=scale)


        p_values.append(max(pval_md, pval_mc))
